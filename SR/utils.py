...
def create_data_lists(train_folders, test_folders, min_size, output_folder):
    ...
    for d in train_folders:
        ...
        for i in os.listdir(d):
            ...
            if img.width >= min_size and img.height >= min_size and quantity > 0:
...
def convert_image(img, source, target):
    ...
    if target == 'pil':
        ...
        # torch.dot() 和 numpy.dot() 的作用不一样
        # 因此使用 torch.matmul() 来计算四维tensor和一维tensor的最后一维的点乘结果
...
class ImageTransforms(object):
    ...
    def __call__(self, img):
        ...
        if self.split == 'train':
            ...
            left = x_remainder // 2
            top = y_remainder // 2
            right = left + (img.width - x_remainder)  # 这里right-left一定是scaling_factor的倍数
